...
開発プレビュー版が PyPI で公開されています：`pip install ppk2lab==0.1.0.dev0`（安定版 `0.1.0` のリリースまでは、単なる `pip install ppk2lab` では何もインストールされません）。開発用途では、以下のようにソースチェックアウトからインストールしてください。
