...
最初の安定版は `0.2.0` です。そこまでに残っているのは実装ではなく検証です。2026-08-20 のハードウェアセッションが 1 個体で macOS のパスを閉じました（ファームウェアフィンガープリント `HW=49625 IA=59.0 keys=40 ports=2`、Apple silicon 上の macOS）。残りの多くは、そのセッションになかったハードウェアを必要とします：
