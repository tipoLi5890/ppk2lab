...
現在のリリースは `0.5.0` で、最初の安定版が `0.2.0` でした。`pip install ppk2lab` だけでインストールできます。experimental と表示しているのは意図的です：ツールチェーン自体はハードウェアなしで十分にテストされ、測定を誠実に保つ性質は仮定ではなく強制されています——しかしその裏付けとなる**ハードウェア検証は部分的**です。1 台のみ、macOS のみ、そしてデコーダはまだ実信号を一度も読んでいません。[ロードマップ](#ロードマップ)が、何を含み何を含まないかを述べています。数値を信頼する前に読んでください。
