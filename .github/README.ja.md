...
現在のリリースは `0.3.0` で、最初の安定版が `0.2.0` でした。`pip install ppk2lab` だけでインストールできます。experimental と表示しているのは意図的です：ツールチェーン自体はハードウェアなしで十分にテストされ、測定を誠実に保つ性質は仮定ではなく強制されています——しかしその裏付けとなる**ハードウェア検証は部分的**です。1 台のみ、macOS のみ、そしてデコーダはまだ実信号を一度も読んでいません。[ロードマップ](#ロードマップ)が、何を含み何を含まないかを述べています。数値を信頼する前に読んでください。
...
`0.3.0` はリリース済みです。残っているのは実装ではなく検証であり、その区別は重要です。2026-08-20 のハードウェアセッションでは、1 個体でツールを端から端まで実行しました（ファームウェアフィンガープリント `HW=49625 IA=59.0 keys=40 ports=2`、Apple silicon 上の macOS）。デバイス探索、metadata、100 kS/s の校正済みキャプチャ、中断からの復旧、すべてのオフラインコマンドを網羅しています。以下は**未検証**であり、それぞれそのセッションになかったハードウェアを必要とします：
