...
`0.3.0` 是目前的版本、`0.2.0` 是第一個穩定版本，因此直接 `pip install ppk2lab` 就能安裝。它刻意標示為 experimental：工具鏈在無硬體的情況下經過大量測試，讓量測保持誠實的那些性質是被強制執行而非假設的——但它背後的**硬體驗證仍然是部分的**。一台機器、只有 macOS，而且還沒有任何解碼器讀過真實訊號。[路線圖](#roadmap)說明它涵蓋了什麼、又沒有涵蓋什麼；在你信任任何一個數字之前，請先讀它。
...
以上指令目前皆已可用。每個 JSON 契約都帶有 `schema_version` 1，並自 `0.2.0` 起遵循 [docs/SPEC.md](https://github.com/tipoLi5890/ppk2lab/blob/main/docs/SPEC.md) 的穩定性政策：新增欄位向後相容，移除或改變語意則需要 `schema_version` 升版與遷移說明。
...
每一次擷取都會回報自己遺失了多少樣本。`0.2.0` 量到的遺失，多數其實是自己造成的：artifact 寫入器在餵入樣本的那條執行緒上壓縮每個 4 MB chunk，那約 290 ms 的停頓塞爆了讀取端的佇列。這正是為什麼那些 60 秒擷取「剛好」各遺失五個整段 chunk——每跨過一個 chunk 邊界就掉一個。現在壓縮跑在自己的執行緒上，讀取佇列也改以位元組（而非讀取筆數）設限；在同一台主機上連續六次擷取（5 × 30 秒與 1 × 60 秒）記下了每一個樣本：零缺口、`complete: true`。主機本身在負載下要付出多少代價，目前無法量化——它從來沒有和寫入器自己的停頓分開過——所以請把遺失視為可能發生、一定會被回報，但不再是預期中的事。
...
`0.3.0` 已發布。仍待完成的是驗證而非實作，而分清楚哪個是哪個很重要。2026-08-20 的一次硬體測試在單一裝置上把工具從頭跑到尾——韌體指紋 `HW=49625 IA=59.0 keys=40 ports=2`，Apple silicon 上的 macOS——涵蓋裝置探索、metadata、100 kS/s 校正後擷取、中斷復原，以及所有離線指令。以下這些**尚未**驗證，而且各自都需要那次測試沒有的硬體：
