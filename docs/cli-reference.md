...
ppk2lab configure --device SERIAL [--mode ampere|source]
                  [--voltage-mv MV] [--dut-power on|off] [--apply]
```
...
Reports stored/missing samples, mean/min/max current, charge (uC), energy
(uJ; `null` when the supply voltage is unknown), and the gaps inside the
window.
